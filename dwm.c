...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - stw - 2 * sp, 0, tw, bh, lrpad / 2 - 2, stext, 0); /* try tw-2sp instead, systray patch */
	}
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		...
		if (showsystray && m == systraytomon(m))
			w -= getsystraywidth();
		m->barwin = XCreateWindow(dpy, root, m->wx, m->by, w - 2 * sp, bh, 0, DefaultDepth(dpy, screen),
